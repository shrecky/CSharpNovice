...
namespace Bit3
{
    class Program
    {
        ...
        {
            //Write a boolean expression for finding if the bit 3(counting from
            //0) of a given integer is 1 or 0.
            ...
            if (addMask != 0)
            {
                Console.WriteLine($"The bit in position {bitPosition} is 1");
            }
            else
            {
                Console.WriteLine($"The bit in position {bitPosition} is 0"); // It isn't 1
            }
            ...
            Console.WriteLine("{0,15} : numbern{1,15} : maskn{2,15} : addMask", numberBinary, maskBinary, addMaskBinary);
